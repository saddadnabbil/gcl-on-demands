<script>
	$(function () {
		...
		$('#datatable').DataTable({
			processing: true,
			serverSide: true,
			ajax: "{{ route('guest.cash-transaction-expenditures.index') }}",
			columns: [
		...
		$('#datatable').on('click', '.cash-transaction-expenditures-detail', function () {
			loadingAlert.show();

			let id = $(this).data('id');
			let url = "{{ route('api.cash-transaction-expenditures.show', 'id') }}";
			...
			$.ajax({
				...
				success: function (response) {
					...
					$('#showCashTransactionModal #expenditure').val(response.data.expenditure);
